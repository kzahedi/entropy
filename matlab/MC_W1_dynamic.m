function [ r ] = MC_W1_dynamic( w2, w1, a1 )
...
for w2index = 1:w2max
    for a1index = 1:a1max
      if pa1(a1index,1) > 0.0
        pw2_c_a1(w2index, a1index) = pw2a1(w2index, a1index) / pa1(a1index,1);
      end
    end
end

values = zeros(w2max, w1max, a1max);

for w2index = 1:w2max
    for w1index = 1:w1max
        for a1index = 1:a1max
            if pw2_c_w1a1(w2index, w1index, a1index) > 0.0 && ...
               pw2_c_a1(w2index, a1index)            > 0.0
           values(w2index, w1index, a1index) = log2( pw2_c_w1a1(w2index, w1index, a1index) / pw2_c_a1(w2index, a1index));
            end
        end
    end
end
...
for t = 1:length(w2)
    ...
    r(t,1) = values(w2index, w1index, a1index);
end
