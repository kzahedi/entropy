...
double 	IT:: prop(int Feati,int Featj,double ValX,double ValY){
	  ...
	  double norm=0;
	  double exponent=0;
	  if(_gis){ exponent= exp((*_FM).getFeatureArrayvalue(Feati,Featj,ValX,ValY) );}
	  else{exponent= exp((*_IM).getFeatureArrayvalue(Feati,Featj,ValX,ValY) );}

	  for(int yi=0;yi<_sizeY;yi++){
		  if(_gis){  norm+= exp( (*_FM).getFeatureArrayvalue(Feati,Featj,ValX,(*_Y)(yi,0)));}
		  else{ norm+= exp( (*_IM).getFeatureArrayvalue(Feati,Featj,ValX,(*_Y)(yi,0))); 	}
	  }
...
double IT ::prop(int rowX,vector<vector<double> > Y, int rowY){
  ...
  for(int Featx=0;Featx< _sizeColValX;Featx++){
	  for(int Featy=0;Featy< _sizeColValY;Featy++){
		  if(_gis){feat+=(*_FM).getFeatureArrayvalue(Featx,Featy,(*_valX)(rowX,Featx),Y[rowY][Featy]);}
		  else{feat+=(*_IM).getFeatureArrayvalue(Featx,Featy,(*_valX)(rowX,Featx),Y[rowY][Featy]);}
  	  }
    }
    exponent= exp(feat);
    for(int yi=0;yi<Y.size();yi++){
	  for(int Featx=0;Featx< _sizeColValX;Featx++){
		  for(int Featy=0;Featy< _sizeColValY;Featy++){
			  if(_gis){featnorm+= (*_FM).getFeatureArrayvalue(Featx,Featy,(*_valX)(rowX,Featx),Y[yi][Featy]);}
			  else{featnorm+= (*_IM).getFeatureArrayvalue(Featx,Featy,(*_valX)(rowX,Featx),Y[yi][Featy]); }
		  }
	  }
...
double IT ::	propm(vector<vector<double> > X,int rowX,vector<vector<double> > Y){
	...
	for(int y=0;y<Y.size();y++){
		  for(int Featx=0;Featx< _sizeColValX;Featx++){
			  for(int Featy=0;Featy< _sizeColValY;Featy++){
				  if(_gis){feat+=(*_FM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[y][Featy]);}
				  else{feat+=(*_IM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[y][Featy]);}
		  	  }
		    }
		  z+=exp(feat);
		  feat=0;
	...
	for(int x=0;x<X.size();x++){
		for(int y=0;y<Y.size();y++){
			  for(int Featx=0;Featx< _sizeColValX;Featx++){
				  for(int Featy=0;Featy< _sizeColValY;Featy++){
					  if(_gis){feat+=(*_FM).getFeatureArrayvalue(Featx,Featy,X[x][Featx],Y[y][Featy]);}
					  else{feat+=(*_IM).getFeatureArrayvalue(Featx,Featy,X[x][Featx],Y[y][Featy]);}
			  	  }
			  }
			  nexp +=exp(feat);
			  feat=0;
...
double IT:: propm(vector<vector<double> > X,int rowX,vector<vector<double> > Y, int rowY){
	  ...
	  for(int Featx=0;Featx< _sizeColValX;Featx++){
		  for(int Featy=0;Featy< _sizeColValY;Featy++){
			  if(_gis){feat+=(*_FM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[rowY][Featy]);}
			  else{feat+=(*_IM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[rowY][Featy]);}
	  	  }
	    }
	   exponent= exp(feat);
	   for(int x=0;x<X.size();x++){
	    for(int yi=0;yi<Y.size();yi++){
		  for(int Featx=0;Featx< _sizeColValX;Featx++){
			  for(int Featy=0;Featy< _sizeColValY;Featy++){
				  if(_gis){ featnorm+= (*_FM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[yi][Featy]);}
				  else{featnorm+= (*_IM).getFeatureArrayvalue(Featx,Featy,X[rowX][Featx],Y[yi][Featy]); }
			  }
		  }
...
double IT::getFeatureArraylambda(int Feati, int Featj, int ilambdaX, int ilambdaY){
  assert(Feati<_sizeColValX && Featj<_sizeColValY);
  double lambda;
  if(_gis){lambda=_FM->getFeatureArraylambda(Feati,Featj, ilambdaX,ilambdaY);}
  else{ lambda=_IM->getFeatureArraylambda(Feati,Featj, ilambdaX,ilambdaY);}
  return lambda;
}
double**** IT:: __getobs(){
